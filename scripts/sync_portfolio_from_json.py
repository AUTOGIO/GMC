#!/usr/bin/env python3
"""
Sync convex_portfolio_instruments.md and dashboard CSVs from gmc_portfolio_state.json.
Run from repo root: python3 scripts/sync_portfolio_from_json.py
"""
import json
import csv
import os
...
PORTFOLIO_JSON = os.path.join(
    REPO_ROOT, "data", "gmc_source", "portfolio", "gmc_portfolio_state.json"
)
DOCS_DIR = os.path.join(REPO_ROOT, "docs")
MD_PATH = os.path.join(DOCS_DIR, "convex_portfolio_instruments.md")
PROCESSED = os.path.join(REPO_ROOT, "data", "processed")
INSTRUMENTS_CSV = os.path.join(PROCESSED, "gmc_convex_instruments.csv")
SUMMARY_CSV = os.path.join(PROCESSED, "gmc_convex_asset_summary.csv")
...
def main():
    ...
    for g in gavetas:
        comps = ", ".join(
            f"{c['asset_class']} {int(c['weight']*100)}%" for c in g.get("components", [])
        )
        gaveta_lines.append({
            "Bucket": g["name"],
    ...
    with open(MD_PATH, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"Source: `gmc_portfolio_state.json` (gmc_convex_2026_q1). "
            f"Total capital: USD {cap:,}. Last update: {as_of}.\n\n"
        )
        f.write("## All instruments (target allocation)\n\n")
        headers = [
            "Asset Class", "Instrument ID", "Name", "Ticker", "Type", "Custodian",
            "Target Weight", "Amount USD", "Currency", "Execution Status", "Notes",
        ]
        ...
        for r in gaveta_lines:
            f.write(
                f"| {r['Bucket']} | {r['Target Weight']} | {r['Target USD']} | {r['Components']} |\n"
            )
